    ...
    <main className="mx-auto flex max-w-7xl flex-col gap-6 p-6">
      <section className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-2">
          <h1 className="bg-gradient-to-r from-primary via-violet-500 to-emerald-500 bg-clip-text text-3xl font-semibold tracking-tight text-transparent">Product enrichment dashboard</h1>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border-primary/20 bg-gradient-to-br from-card to-violet-50/80"><CardHeader><CardTitle className="text-primary">{products.length}</CardTitle><CardDescription>Products in current fixture</CardDescription></CardHeader></Card>
        <Card className="border-emerald-300/60 bg-gradient-to-br from-card to-emerald-50/80"><CardHeader><CardTitle className="text-emerald-700">{products.reduce((acc, p) => acc + p.candidates.length, 0)}</CardTitle><CardDescription>Candidate improvements</CardDescription></CardHeader></Card>
        <Card className="border-destructive/20 bg-gradient-to-br from-card to-red-50/70"><CardHeader><CardTitle className="text-destructive">{products.filter((p) => p.warnings.length > 0).length}</CardTitle><CardDescription>Products with baseline warnings</CardDescription></CardHeader></Card>
      </div>
      <Card>
