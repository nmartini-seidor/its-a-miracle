...
test('schemas and hero fixtures use canonical field identifiers', () => {
  ...
  assert.equal(heroProduct.baselineDescription, '')
  assert.equal(heroProduct.baselineAttributes.description, null)
  assert.equal(heroProduct.sourceUrl.startsWith('https://www.mediamarkt.es/'), true)
...
test('seeded catalog uses electronics imports instead of unrelated retail products', () => {
  ...
  assert.equal(catalogItems.every((product) => product.sourceUrl?.startsWith('https://www.mediamarkt.es/')), true)
