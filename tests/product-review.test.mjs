...
test('hero product baseline includes product name and product detail uses icon tabs with export action', async () => {
  ...
  // Phase 5: the Candidates tab now shows competing candidates with per-runner provenance and a
  // Conflict/Consensus indicator, so candidate confidence IS surfaced and the grouping helpers changed.
  ...
  assert.equal(productPageSource.includes('conflictFieldGroups.map'), true)
  assert.equal(productPageSource.includes('getCompetingCandidatesByField(product)'), true)
  assert.equal(productPageSource.includes('isFieldConflicted(group.candidates)'), true)
  ...
  assert.equal(syncButtonSource.includes('Sync with Mirakl'), false)
