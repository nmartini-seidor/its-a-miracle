...
test('schemas overview stays table-first and links to per-schema configuration pages', async () => {
  ...
  assert.equal(formSource.includes('href={`/products/${product.id}`}'), true)
  assert.equal(formSource.includes('Open product'), true)
  assert.equal(formSource.includes('divide-y divide-slate-200 overflow-hidden rounded-2xl border border-slate-200 bg-white'), false)
