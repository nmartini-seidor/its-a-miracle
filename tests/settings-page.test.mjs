...
test('settings page source advertises tabbed configurable sections', async () => {
  ...
  assert.equal(tabsSource.includes('Operator API key'), true)
  assert.equal(tabsSource.includes('Operator API credential used by the server for Mirakl catalog and product workflow requests.'), true)
  assert.equal(tabsSource.includes('credential placeholder'), false)
  assert.equal(tabsSource.includes('value="************" readOnly'), true)
  assert.equal(tabsSource.indexOf('Mirakl base URL') < tabsSource.indexOf('Operator API key'), true)
